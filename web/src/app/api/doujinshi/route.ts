...
export async function GET(request: NextRequest): Promise<NextResponse<Doujinshi[]> | undefined> {
  ...
  // Send request
  const apiResponse = await axios.get<Doujinshi[]>(`${process.env.API_URL}/api/doujinshi${query}`);
